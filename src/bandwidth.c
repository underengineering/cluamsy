...
static volatile short bandwidthEnabled = 0, bandwidthInbound = 1,
                      bandwidthOutbound = 1;
...
int32_t crate_stats_calculate(CRateStats *rate, uint32_t now_ts) {
    ...
    if (rate->initialized == 0 || rate->sample_num <= 0 || active_size <= 1 ||
        active_size < rate->window_size) {
        return -1;
    }

    r = ((((float)rate->accumulated_count) * rate->scale) / rate->window_size) +
        0.5f;
