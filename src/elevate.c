...
BOOL tryElevate(HWND hWnd, BOOL silent) {
    ...
    if (!IsWindowsVistaOrGreater()) {
        if (!silent)
            MessageBox(hWnd,
                       (LPCSTR) "Unsupported Windows version. clumsy only "
                                "supports Windows Vista or above.",
        ...
        if (GetModuleFileName(NULL, (LPSTR)szPath, ARRAYSIZE(szPath))) {
            ...
            if (!ShellExecuteEx(&sei)) {
                ...
                if (dwError == ERROR_CANCELLED) {
                    ...
                    // alert and exit
                    MessageBox(hWnd,
                               (LPCSTR) "clumsy needs to be elevated to work. "
                                        "Run as Administrator or click Yes in "
                                        "promoted UAC dialog",
                               (LPCSTR) "Aborting", MB_OK);
                }
            }
            ...
            MessageBox(hWnd,
                       (LPCSTR) "Failed to get clumsy path. Please place the "
                                "executable in a normal directory.",
                       (LPCSTR) "Aborting", MB_OK);
        }
    }
